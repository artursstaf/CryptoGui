...
class Aes {
    ...
    private fun runRounds(k: String, m: String, revKeys: Boolean): String {
        history.clear()
        history.add(AesState("Initial values", getBitSet(k), getBitSet(m)))
        ...
        history.add(AesState("Initial permutation", null, message))
        ...
        for (round in 0..15) {
            ...
            history.add(AesState("Round ${round + 1}", keys[round], combineBitSets(left, right)))
        }

        // swap before final permutation
        message = combineBitSets(left = right, right = left)
        history.add(AesState("Swap halves", null, message))

        message = message.finalPermutation()
        history.add(AesState("Final permutation", null, message))
...
fun main() {
    ...
    println()
    println("Decrypted message: ${aes.decrypt(key, encryptedMessage)}")
